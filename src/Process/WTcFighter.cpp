...
if(miState==FIGHTER_FLEE)
{
  ...
  if(mpHull->AngleToVector(lfPlayer)<0.05f)  mpHull->RotateZ(ClampValue(mpHull->RollToVectorPitch(lfPlayer),0.03f));
  //Pitch the fighter so it is inline with the flee vector.
  mpHull->RotateX(ClampValue(mpHull->PitchToPoint(lfPlayer),0.01f));

  //Increment the counter for timing how long the fighter has been fleeing.
  mfFleeingTime+=0.004f;
...
if(miState==FIGHTER_ATTACK)
{
 ...
 if(mpTrack->CanHit())
 {
    //Try to align the ship to the targeting vector
 	mpHull->RotateZ(ClampValue(mpHull->RollToVectorPitch(mpTrack->InterceptionVector()),0.03f));
 	mpHull->RotateX(ClampValue(mpHull->PitchToVector(mpTrack->InterceptionVector()),0.01f));
 ...
 else
 {
    ...
    mpHull->RotateZ(ClampValue(mpHull->RollToPointPitch(cPlayer::HullMatrix.Position()),0.03f));
    mpHull->RotateX(ClampValue(mpHull->PitchToPoint(cPlayer::HullMatrix.Position()),0.01f));
 }
